...
## ExpSystem fired leveled_up → 큐에 쌓고 아직 화면이 없으면 _show_next_levelup 시작.
...
func _on_upgrade_card_selected(card_id: String) -> void:
	...
	_selected_cards.append({"id": card_id, "name": card_name, "skill_type": String(card.get("skill_type", "")), "desc": String(card.get("desc", "")), "pool": String(card.get("pool", "")), "rarity": String(card.get("rarity", ""))})
